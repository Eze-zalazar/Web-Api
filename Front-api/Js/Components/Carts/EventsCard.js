...
export const createEventCard = (event, onSelect) => {
    ...
    card.innerHTML = `
        <div class="event-card-header h-32 p-4 flex flex-col justify-between relative">
            <span class="bg-white/20 backdrop-blur-md text-white text-[10px] uppercase font-bold px-2 py-1 rounded-full w-fit">
                ${event.status || 'On Sale'}
            </span>
            <p class="text-white/80 text-xs font-bold tracking-widest uppercase">${event.venueCity || 'CIUDAD'}</p>
        </div>

        <div class="p-5 flex flex-col flex-grow">
            <h3 class="font-bold text-lg text-slate-800 mb-2 leading-tight">${event.name}</h3>
            ...
            <div class="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between">
                <div>
                   ...
                   <p class="font-bold text-slate-800">${event.price ? event.price.toLocaleString('es-AR', { style: 'currency', currency: 'ARS' }) : 'Ver precios'}</p>
                </div>
    ...
    // Event listener para navegar al mapa de asientos
    card.querySelector('.select-seats-btn').addEventListener('click', () => onSelect(event.id));
